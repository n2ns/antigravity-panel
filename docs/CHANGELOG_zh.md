...
- **配额重置倒计时与通知**：侧边栏倒计时现在基于服务端已提供的绝对重置时间戳在轮询间隔内实时滴答。当某配额池余量回升超过重置阈值（5pp）时会弹出通知（开关：`tfa.system.notifyOnQuotaReset`，带冷却保护以抵御服务端数据抖动）。另有一个对准最近一次重置时刻的一次性定时器触发立即刷新，重置在数秒内即可反映到面板，而无需等待下一个轮询周期。服务端返回无效重置时间时现在会被标记并显示为 N/A，不再合成 24 小时假倒计时。
- **异常消耗告警**：配额在 IDE 关闭期间下降至少 5pp，或在窗口持续失焦且无编辑活动时下降至少 5pp，现在可触发可选警告。检测会感知重置，并按配额池应用通知冷却以减少误报（开关：`tfa.system.notifyOnAbnormalDrain`）。
- **周用量本地估算**：侧边栏现在可显示 7 根日用量柱、当前 7 天累计，以及用于对比的前 7 天累计（开关：`tfa.dashboard.showWeeklyCard`）。未采样时段与零消耗明确区分，卡片也明确说明该指标是短期配额池的本地估算，并非 Google 官方周限额。

### 变更

- **依赖更新**：mocha 更新至 11.7.6，eslint 更新至 10.8.0，lint-staged 更新至 17.2.0，其余开发依赖也在声明范围内刷新。新增仅作用于 mocha 依赖树的 npm overrides（diff、glob、minimatch、brace-expansion），清零 `npm audit` 报告的全部 5 个漏洞（4 高危 1 低危，均为 DoS 类且仅影响开发工具链；生产依赖树此前即无漏洞）。
- **用量历史在配额重置后不再丢失**：检测到重置时不再删除该组的配额历史，而是在记录点上写入重置标记。消耗速率（pp/h）和 runway 预测仍从标记处重新开始——差值计算永不跨越重置点——同时保留 14 天历史，以支持当前 7 天与前 7 天对比。超过 24 小时的历史点降采样为 5 分钟粒度（标记保留）以控制存储体积。存储的 `tfa.quotaHistory_v2` 格式除新增 `resets` 字段外保持不变。
- **运行时与 Payload 清理**：核对生产调用链后，移除了过时的缓存、存储、Automation、Scheduler、状态栏、配额和 Webview DTO 字段与辅助方法。用量图现在直接根据实际渲染的时间桶计算纵轴比例，同时继续使用每个数据项携带的颜色，保持现有图表显示效果。
- **Language Server 调试工具**：真实服务器诊断工具统一归入 `scripts/debug`，并补充明确的 npm 命令与文档。配额工具会输出完整真实响应；当 Linux socket 工具不可用时，可通过目标进程的 `/proc` socket 描述符解析监听端口。
...
- **缓存优先启动的树元数据**：任务与上下文条目现在会持久化并恢复原始字节数和修改时间，IDE 重启后的初始大小显示与排序顺序保持正确。
- **配额失败日志**：恢复生产环境中的错误回调注册，配额请求失败时会写入扩展日志，不再被静默丢弃。
...
- **状态栏连接失败状态**：启用配额监控时，如果 Language Server 连接失败，状态栏现在会切换到警告状态，不再继续显示过期配额；仅显示缓存的状态栏模式仍可独立工作。
...
- **Gemini Flash 与 Pro 分组修正** (#127): 修复了 Flash 模型（通过 ID 或标签识别）被错误归入 Gemini Pro 组的问题。前缀匹配现采用最长前缀优先策略，并对泛化 `gemini` 前缀增加了 Flash 显式排除守卫。
- **侧边栏标题**: Activity Bar 容器标题更新为 "Antigravity Toolkit"。
- **配额重置窗口对齐** (#128): 将固定的 5 小时预测窗口替换为从各模型组实际 `resetTime` 字段动态获取的值，API 返回无效时回退到 24 小时。
- **时间格式扩展** (#128): 状态栏与图表剩余时间现支持多周格式（如 `2w 3d`、`~3w`），不再仅以天数截止。
- **Claude + GPT 共享配额池** (#129): Claude 与 GPT 行共享同一后端配额池。剩余百分比、重置时间及图表预测现统一取两组中的最低值，避免重复计算。
